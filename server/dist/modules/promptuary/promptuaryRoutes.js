...
async function promptuary(app) {
    app.post('/webhook/endpoint/path/', async (request, reply) => {
        ...
        const protocolOnMediflowObjectRequestSenha = {
            method: 'POST',
        ...
        const medflowBundleSenha = (await (0, makeRequest_1.makeRequest)(protocolOnMediflowObjectRequestSenha)).data;
        ...
        if (toGetStatus == 'FINALIZADO') {
            ...
            for (const resource of resourcesToProcess) {
                ...
                let ie_tipo_param = '';
                if (resourceP) {
                    ie_tipo_param = resource?.resource?.category[0].text;
                }
